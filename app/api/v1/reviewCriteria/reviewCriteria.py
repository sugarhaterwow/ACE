...
async def list(
    ...
    file_path = os.path.join(settings.MOCKDATA_ROOT, "review-criteria.json")
...
@router.delete("/{id}", summary="模拟删除审查项")
async def delete_review_item(id: str = Path(..., description="要删除的项 ID")):
    # 模拟删除逻辑：不真正修改文件，只返回成功
    print(f"模拟删除项：{id}")
    return Success(msg=f"Item {id} deleted (mock)", data={"id": id})
